...
This will be our first single page app. Our task, after we pull from the
...
- When the user selects an article's title show the popup overlay. The
  content of the article should be inserted inside the popup.
...
- When the app is first loading and when the user selects to load a new feed
  from the dropdown, display a loading indicator - the `.loader` container already includes styles
  for this by applying the `.loader` class.
...
* A __new git repository hosted on Github__, where your codebase is maintained.
  ...
  the index.html and css files if you would like to further customize your
  app.
...
Begin by creating a new application with `create-react-app`.  Once
complete, add the following assets to your application to get some initial structure and styles

You can then open Atom or your favorite editor and point it at application directory and
